...
public class FinalPlayer extends Player {
						...
						if (fault == 0) {
							for (int y1 = 0; y1 < getHeight(); y1++) {
								for (int x1 = 0; x1 < getWidth(); x1++) {
									...
									if (0 < virtual[x1][y1] && virtual[x1][y1] < 9) {
										if (virtual[x1][y1] == virflagNum(x1,y1)) {
											for (int dy = -1; dy <= 1; dy++) {
												for (int dx = 0; dx <= 1; dx++) {
													...
													if (virtual[x1 + dx][y1 + dy] == -1) {
														countdamy[x1+dx][y1+dy]=countdamy[x1+dx][y1+dy]+1;
													}
												}
											}
										}
