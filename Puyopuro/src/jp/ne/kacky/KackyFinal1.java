...
import kajiGod.KajiField;

public class KackyFinal1 extends AbstractPlayer {

	public KackyFinal1(String playerName) {
		super(playerName);
    ...
    private int getScore(Field field)
    {
        int pointFinal = 0;
        int pointConnect = 0;
        int pointIsolateMinus = 0;
        int pointOver = 0;
        int pointFire = 0;
        int pointDivMinus = 0;
        int pointDivTopMinus = 0;
        ...
        for(int c = 0; c < 6; c++)
        {
            ...
            {
            	//つながりが
                pointConnect = (int)((double)pointConnect + Math.pow(connectPuyo[c][r], 3D));//pow(a,b):aのb乗
                if(connectPuyo[c][r] == 1)
                {
                    int isolateRoom = 0;
                    if(r < 13 && connectPuyo[c][r + 1] == 0)
                        isolateRoom++;
                    if(c >= 1 && connectPuyo[c - 1][r] == 0)
                        isolateRoom++;
                    if(c <= 4 && connectPuyo[c + 1][r] == 0)
                        isolateRoom++;
                    switch(isolateRoom)//ひとつだけのぷよを、周囲を違う色で囲まれているほどマイナスにする。まわりに何もないとマイナスしない。
                    {
                    case 0: // '\0'
                        pointIsolateMinus -= 100;
                        break;
                ...
                if(connectPuyo[c][r] == 3)//同色３つの塊のとき
                {
                    for(int c2 = c - 1; c2 <= c + 1; c2++)//自身と両隣
                        if(c2 != -1 && c2 != 6)
                        {
                            boolean countOverFlag = false;
                            for(int r2 = r; r2 < 14; r2++)//自身以上
                                if(countOverFlag)
                                {
                    ...
                    if(c != 0 && connectPuyo[c - 1][r] == 0 && field.getTop(c - 1) + 2 <= r)
                        countFireFrag = true;
                    if(c != 5 && connectPuyo[c + 1][r] == 0 && field.getTop(c + 1) + 2 <= r)
                        countFireFrag = true;
                    if(countFireFrag)
        ...
        if((field.getTop(2) + field.getTop(3)) * 3 < sumPuyoOnField)//中心２列に集まるようにポイントー
